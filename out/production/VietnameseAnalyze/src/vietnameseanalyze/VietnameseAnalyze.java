...
public class VietnameseAnalyze
{
...
//    }
    ...
    public static ArrayList<Double> getWaitingEdgeLeastSquares(HashMap<EdgeVertex, Double> EdPR, HashMap<Object, Double> PR)
    {
        ...
        for (var parser : VietnameseAnalyze.Parsers)
        {
            var auxg = getDependenceGraph(parser);
                ...
                for (var iType : iTypes)
                {
                    ...
                    for (var jType : jTypes)
                    {
                        ...
                        if (EdPR.containsKey(edV))
                        {
                            ...
                            double idealWeight;
                            if (direction.equals("next"))
                                idealWeight = VietnameseAnalyze.nextVertexToEdgeRankLeastSquares.get(0) * weight0 + VietnameseAnalyze.nextVertexToEdgeRankLeastSquares.get(1) * weight1;
                            else
                                idealWeight = VietnameseAnalyze.beforeVertexToEdgeRankLeastSquares.get(0) * weight0 + VietnameseAnalyze.beforeVertexToEdgeRankLeastSquares.get(1) * weight1;
                        ...
                        } else
                        {
                            ...
                            var weight1 = PR.get(jPair);
                            if (direction.equals("next"))
                                weight = VietnameseAnalyze.nextVertexToEdgeRankLeastSquares.get(0) * weight0 + VietnameseAnalyze.nextVertexToEdgeRankLeastSquares.get(1) * weight1;
                            else
                                weight = VietnameseAnalyze.beforeVertexToEdgeRankLeastSquares.get(0) * weight0 + VietnameseAnalyze.beforeVertexToEdgeRankLeastSquares.get(1) * weight1;
                        }
                        {
...
//        forbiddenTypeConnections.add(new CustomArrayList<String>(new String[]{null, "A", "V"}));
    ...
    {
        var obj = new VietnameseAnalyze();
        VietnameseAnalyze.Parsers = obj.getDataFromFile();
        getWordAppearances();
        VietnameseAnalyze.getErrorAppearanceStatics();
        VietnameseAnalyze.isInitialized = true;
    }
